    ...
    where T: Debug
{
    fn display(&self) {
        //看看这两个调用意思，是什么意思了。
        println!("----dispaly---{:?}---", *self); //这一次是对self本身时引用类型，先对它解引用，变成所有者，
        println!("----dispaly---{:?}---", **self); //显示的调用*操作符。
    }
}
...
fn main() {
    ...
    println!("-----------");
    println!("---*x = {:?}---", *x); //这里对一个所有者解引用，编译器发现存在自定义的deref，会进行先调用deref()，所以时char类型，

    println!("---&x = {:?}---", &x); //这里不必多说了。DerefExample类型
    foo(&x); //这个时编译器的自动转换，由＆DerefExample类型转换成　&char类型。

    println!("---&*&x = {:?}---", *&x); //这里是DerefExample类型 为什么呢？因为
